...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	static final boolean USE_HW_LAYERS = false; // 使用硬件加速（会非常消耗系统性能）
	static final float FRICTION = 2.0f; // 滑动阻力倍数
	...
	// WGX 20160426
	private CharSequence labelDownPull;
	private CharSequence labelDownRelease;
	private CharSequence labelDownRefreshing;
	private CharSequence labelUpPull;
	private CharSequence labelUpRelease;
	private CharSequence labelUpRefreshing;
	...
	private SharedPreferences mLastUpdateTimePreferences;
	private String LAST_UPDATE_TIME_PREFERENCES_FILENAME = "pull_to_refresh_view_last_update_time";
	private String mLastUpdataTimeKey;
	...
	/**
	 * 设置记录该ListView最后刷新时间的唯一标识key，该key可以使用HTTP的请求地址加密后作为key。同时更新最后一次刷新的时间
	 * @param key
	 */
	public void setKey(String key){
		this.mLastUpdataTimeKey = key;
	 ...
	 * 显示或更新刷新时间
	 */
	private void setLastUpdateLable(){
		String label = null;
		long curUpdataTime = System.currentTimeMillis();
		if(mLastUpdateTime == -1){
			mLastUpdateTime = getLastUpdateTime();
		}
		long len = curUpdataTime - mLastUpdateTime;
		if(len < 60000 * 1){
			label = "刚刚";
		} else if (len >= 60000 * 1 && len < 60000 * 5){
			label = (int)(len/60000) + "分钟前";
		} else if (len >= 60000 * 5 && len < 60000 * 10){
			label = "5分钟前";
		} else if (len >= 60000 * 10 && len < 60000 * 20){
			label = "10分钟前";
		} else if (len >= 60000 * 20 && len < 60000 * 30){
			label = "20分钟前";
		} else if (len >= 60000 * 30 && len < 60000 * 60){
			label = "30分钟前";
		} else if (len >= 60000 * 60 * 1 && len < 60000 * 60 * 2){
			label = "1小时前";
		} else if (len >= 60000 * 60 * 2 && len < 60000 * 60 * 3){
			label = "2小时前";
		} else {
			label = DateUtils.formatDateTime(getContext(), curUpdataTime, DateUtils.FORMAT_SHOW_TIME //
				| DateUtils.FORMAT_SHOW_DATE | DateUtils.FORMAT_ABBREV_ALL);
		}
		if(mLoadingLayoutProxy == null){
			mLoadingLayoutProxy = getLoadingLayoutProxy(true, false);
//			mLoadingLayoutProxy.setLastUpdatedLabel(null);
		}
		mLoadingLayoutProxy.setLastUpdatedLabel(String.format("最后更新: %s", label));
	}
	...
	public void setLastUpdateTime(long time){
		...
		getPreferences().edit().putLong(mLastUpdataTimeKey, time).commit();
	}
	
	private long getLastUpdateTime(){
		return getPreferences().getLong(mLastUpdataTimeKey, -1);
	}

	private SharedPreferences getPreferences(){
		if(mLastUpdateTimePreferences == null){
			mLastUpdateTimePreferences = getContext().getSharedPreferences(LAST_UPDATE_TIME_PREFERENCES_FILENAME , Context.MODE_PRIVATE);
		}
		return mLastUpdateTimePreferences;
	}
	// =======My end====================================================
	...
	public void addView(View child, int index, ViewGroup.LayoutParams params) {
		...
		if (refreshableView instanceof ViewGroup) {
			...
			throw new UnsupportedOperationException("Refreshable View不是ViewGroup，所以不能添加View。Refreshable View is not a ViewGroup so can't addView");
		}
	}
		...
		switch (action) {
			...
			case MotionEvent.ACTION_MOVE: {
				...
				if (isReadyForPull()) {
					...
					switch (getPullToRefreshScrollDirection()) { // 判断滑动方向
						case HORIZONTAL: //水平
							diff = x - mLastMotionX;
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_UP: {
				if (mIsBeingDragged) {
					...
					// 如果已刷新，立刻回滚到顶部。If we're already refreshing, just scroll back to the top
					if (isRefreshing()) {
//						smoothScrollTo(0, 500, 0, null);
						smoothScrollTo(0);
						return true;
					}

					// If we haven't returned by here, then we're not in a state to pull, so just reset
					setState(State.RESET);
	...
	/**
	 * 设置RefreshView上拉或下拉时是否显示header和footer的文字和图标
	 * @param visible 同时控制header和footer的文字和图标显示状态
	 */
	public void setLabel(boolean visible){
		setLabel(visible, visible);
	}

	/**
	 * 设置RefreshView下拉或下拉时是否显示header和footer的文字和图标
	 * @param downVisible 显示header的文字和图标
	 * @param upVisible 显示footer的文字和图标
	 */
	public void setLabel(boolean downVisible, boolean upVisible){
		// 下拉
		if(downVisible){
			getLoadingLayoutProxy(true, false).setPullLabel(labelDownPull); // 刚下拉时，显示的提示
			getLoadingLayoutProxy(true, false).setReleaseLabel(labelDownRelease); // 下来达到一定距离时，显示的提示
			getLoadingLayoutProxy(true, false).setRefreshingLabel(labelDownRefreshing); // 刷新时
		} else {
			getLoadingLayoutProxy(true, false).setLoadingDrawable(null);
			getLoadingLayoutProxy(true, false).setPullLabel(" ");
			getLoadingLayoutProxy(true, false).setReleaseLabel(" ");
			getLoadingLayoutProxy(true, false).setRefreshingLabel(" ");
		}
		// 上拉
		if(upVisible){
			getLoadingLayoutProxy(false, true).setPullLabel(labelUpPull);
			getLoadingLayoutProxy(false, true).setReleaseLabel(labelUpRelease);
			getLoadingLayoutProxy(false, true).setRefreshingLabel(labelUpRefreshing);
		} else {
			getLoadingLayoutProxy(false, true).setLoadingDrawable(null);
			getLoadingLayoutProxy(false, true).setPullLabel(" ");
			getLoadingLayoutProxy(false, true).setReleaseLabel(" ");
...
//					pTop = 0; // TODO 设置为0，可在预览中显示正常,但APP内会距上 maximumPullScroll 距离
	...
	private void init(Context context, AttributeSet attrs) {
		...
		// WGX 20160426
		labelDownPull = getResources().getString(R.string.pull_to_refresh_pull_label);
		labelDownRelease = getResources().getString(R.string.pull_to_refresh_release_label);
		labelDownRefreshing = getResources().getString(R.string.pull_to_refresh_refreshing_label);
		labelUpPull = getResources().getString(R.string.pull_to_refresh_from_bottom_pull_label);
		labelUpRelease = getResources().getString(R.string.pull_to_refresh_from_bottom_release_label);
		labelUpRefreshing = getResources().getString(R.string.pull_to_refresh_from_bottom_refreshing_label);
		...
		if (newScrollValue != 0 && !isRefreshing()) {
			...
			switch (mCurrentMode) {
				...
				default:
					...
					setLastUpdateLable(); // TODO WGX Add 20160301 更新刷新时间
					break;
			}
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		public void run() {
			...
			if (mStartTime == -1) {
				...
				// ********************My start**************************
				// 防止mDuration时报被除数为0异常
				long noZeroDuration = 1;
				if(mDuration != 0){
					noZeroDuration = mDuration;
				}
				// ********************My end**************************
				/**
...
//				long normalizedTime = (1000 * (System.currentTimeMillis() - mStartTime)) / mDuration;
				long normalizedTime = (1000 * (System.currentTimeMillis() - mStartTime)) / noZeroDuration;

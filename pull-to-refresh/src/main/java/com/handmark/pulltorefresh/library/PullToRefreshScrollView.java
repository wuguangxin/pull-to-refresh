...
public class PullToRefreshScrollView extends PullToRefreshBase<ScrollView>{
	...
	public PullToRefreshScrollView(Context context, Mode mode, ViewGroup.LayoutParams params){
		super(context, mode);
	...
	public void setLoadingLayoutBackground(Drawable drawable){
		...
		if(getFooterLayout() != null) ViewCompat.setBackground(getFooterLayout(), drawable);
	}

	/**
	 * 设置上拉时footer和下拉时header的背景资源。wgx:20161230
	 * @param resource
	 */
	public void setLoadingLayoutBackgroundResource(int resource){
		if(getHeaderLayout() != null) getHeaderLayout().setBackgroundResource(resource);
		if(getFooterLayout() != null) getFooterLayout().setBackgroundResource(resource);
	}
	...
	final class InternalScrollViewSDK9 extends ScrollView{
		...
		private int getScrollRange(){
			...
			if (getChildCount() > 0) {
				...
				// 上句修改如下
//				scrollRange = Math.max(0, child.getHeight() - getHeight()); //让滑动距离减半
			}
			return scrollRange;
		}
	}
